...
class _AccountSettingsScreenState extends State<AccountSettingsScreen> {
  ...
  Future<void> _saveUserSettings() async {
    ...
    try {
      ...
      print('メールアドレスを保存しました: ${_emailController.text}');
      
      // パスワードの保存処理を強化
      if (_passwordController.text.isNotEmpty) {
        // パスワードを確実に保存
        final success = await prefs.setString('userPassword', _passwordController.text);
        
        if (success) {
          print('パスワードを正常に保存しました: ${_passwordController.text}');
        } else {
        ...
        final verifyPassword = prefs.getString('userPassword');
        if (verifyPassword != _passwordController.text) {
          print('パスワードの検証に失敗: 期待="${_passwordController.text}", 実際="$verifyPassword"');
          throw Exception('パスワードの検証に失敗しました');
        }
      }
      ...
      final savedNewsletter = prefs.getBool('newsletterEnabled');
      print('保存確認 - メール: $savedEmail, パスワード: $savedPassword, ニュースレター: $savedNewsletter');
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  // パスワード入力フィールド
                  _buildInputSection('パスワード', _passwordController, true),
                  
                  // パスワード確認フィールド
                  _buildInputSection('パスワード確認', _confirmPasswordController, true),
